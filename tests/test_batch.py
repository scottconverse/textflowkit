"""Batch orchestration over the single-item runner."""
...
def test_batch_reports_every_item_and_continues_after_failure(monkeypatch):
    from textflowkit.core import batch
    ...
    monkeypatch.setattr(batch, "run_job", fake_run_job)
...
def test_batch_resume_reuses_done_result_without_new_job(monkeypatch):
    ...
    monkeypatch.setattr(batch, "run_job", explode)
